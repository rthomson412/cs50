...
int compute_score(string word)
{
    ...
    for (int i = 0; i < strlen(word); i++)
    {
        ...
        if (isupper(word[i]))
        {
            // if letter is upper find words from capital_letters array
            // if capital letter is found assign points to temp_Points array
            // adds tem_points value to int score to calculate total score
        ...
        {
            // Same process as above for but for lower letters
        ...
        // Ignores anything in word not found in alphabets
        else
        {
            i += 1;
        }
